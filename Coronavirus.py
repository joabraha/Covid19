...
def UKaggregate(mapping,df,sumavg,datapoint,date):
    ...
    dfreturn = {'regionNUTS' : listmapping,
...
'Norte' : ['Aveiro District', 'Braga', 'Bragança District', 'Porto District', 'Viana do Castelo District', 'Vila Real District'],
'Alentejo': ['Beja District', 'Évora District', 'Portalegre District', 'Setubal'],
'Centro': ['Castelo Branco District', 'Coimbra District', 'Guarda District', 'Leiria District', 'Santarém District', 'Viseu District'],
'Lisbon': ['Lisbon'],
...
dfCriticalCareBeds = pd.DataFrame({
    'Country':['Germany','Luxemburg','Austria','Belgium','Italy','France','Spain','Norway','Denmark','UK','Netherlands','Finland','Sweden','Portugal','Poland'],
    'CriticalCareBeds':[29.2,24.8,21.8,15.9,12.5,11.6,9.7,8.0,6.7,6.6,6.4,6.1,5.8,4.2,6.9]
    })
...
dfGDP = pd.read_csv('Eurostat Data\Eurostat GDP/nama_10r_2gdp_1_Data.csv', encoding = "ISO-8859-1")
...
dfGlobalMobility.loc[dfGlobalMobility['sub_region_1'].isna(),'sub_region_1'] = dfGlobalMobility.loc[dfGlobalMobility['sub_region_1'].isna(),'country_region']

dfPortGMagg = GoogMobaggregate(mapPortugalDistrRegion, dfGlobalMobility,'Portugal','PT')
...
dfUK_final['NHS England Region'] = dfUK_final['NHS England Region'].str.replace('North West','North West (UK)')
    #Sum until this date
...
dfSweden_final = dfSweden.drop(['Region', 'Population', 'Lat','Long','Today', 'Diff', 'Region_Deaths',
       'FHM_Deaths_Today'], axis=1)
...
listcountries = ['Austria','Luxemburg','Norway','Poland','Denmark','Netherlands','Finland']
dfdata_country = dfdata[dfdata['CountryName'].isin(listcountries) & dfdata['RegionName'].isna()]
...
dfPopulation_final = pd.concat([UKaggregate(mapUKNutsNHS,dfPopulation_final,'sum','Population',2019), dfPopulation_final]).drop_duplicates(subset=['regionNUTS'],keep='first')
...
dfPopulationDensity_final['Value'] = pd.to_numeric((dfPopulationDensity_final['Value'].str.replace(',','')).str.replace(':',''))
    #Delete unnecessary columns and empty rows
dfPopulationDensity_final = dfPopulationDensity_final[['TIME','GEO','Value']].dropna()
    #Only Keep latest entry of every region
dfPopulationDensity_final = dfPopulationDensity_final.sort_values('TIME',ascending=False)
dfPopulationDensity_final.columns = ['Year_PopulationDensity','regionNUTS','PopulationDensity']
    #UK Specific aggregation
dfPopulationDensity_final = pd.concat([UKaggregate(mapUKNutsNHS,dfPopulationDensity_final,'avg','PopulationDensity',2018),dfPopulationDensity_final]).drop_duplicates(subset=['regionNUTS'],keep='first')
...
#GDP
    #Problem with Points (633.383,789) as string -> not identified
dfGDP_final = dfGDP
dfGDP_final['Value'] = pd.to_numeric((dfGDP_final['Value'].str.replace(',','')).str.replace(':',''))
    #Delete unnecessary columns and empty rows
...
dfGDP_final = pd.concat([dfGDP_Sweden, UKaggregate(mapUKNutsNHS,dfGDP_final,'sum','GDP',2018),dfGDP_final]).drop_duplicates(subset=['regionNUTS'],keep='first')
...
dfHospitalBeds_final['Value'] = pd.to_numeric(dfHospitalBeds_final['Value'],errors='coerce')
    #Delete unnecessary columns and empty rows
dfHospitalBeds_final = dfHospitalBeds_final[['TIME','GEO','Value']].dropna()
    #Only Keep latest entry of every region
dfHospitalBeds_final = dfHospitalBeds_final.sort_values('TIME',ascending=False).drop_duplicates(subset=['GEO'])
...
ListCountrydf = [dfBelgium_final,dfItaly_final,dfSpain_final,dfGermany_final,dfUK_final,dfFrance_final,dfSweden_final,dfPortugal_final,dfdata_country]
...
dfDeathdata = pd.merge(dfDeathdata, dfPopulationDensity_final, how='left', on='regionNUTS')
...
dfDeathdata = pd.merge(dfDeathdata, dfGlobalMobility, how='left', left_index=True, right_index=True)
...
dfDeathdata['personsperhousehold'] = dfDeathdata['Population'] / dfDeathdata['Households'] / 1000
...
dfDeathdata['chgdeathsper1m_shifted'] = dfDeathdata.groupby('region')['chgdeathsper1m'].shift(-24)

dfDeathdata['deathsper1m_shifted'] = dfDeathdata.groupby('region')['deathsper1m'].shift(-24)
#3day moving average
...
dfDeathdata['retail_and_recreation_percent_change_from_baseline3dma'] = dfint['retail_and_recreation_percent_change_from_baseline']
dfDeathdata['grocery_and_pharmacy_percent_change_from_baseline3dma'] = dfint['grocery_and_pharmacy_percent_change_from_baseline']
dfDeathdata['parks_percent_change_from_baseline3dma'] = dfint['parks_percent_change_from_baseline']
dfDeathdata['transit_stations_percent_change_from_baseline3dma'] = dfint['transit_stations_percent_change_from_baseline']
dfDeathdata['workplaces_percent_change_from_baseline3dma'] = dfint['workplaces_percent_change_from_baseline'] 
dfDeathdata['residential_percent_change_from_baseline3dma'] = dfint['residential_percent_change_from_baseline']

dfDeathdata['intDeathsandretail_and_recreation_percent_change_from_baseline3dma'] = dfDeathdata['retail_and_recreation_percent_change_from_baseline3dma']*dfDeathdata['deathsper1m_shifted']
dfDeathdata['intDeathsandtransit_stations_percent_change_from_baseline3dma'] = dfDeathdata['transit_stations_percent_change_from_baseline3dma']*dfDeathdata['deathsper1m_shifted']
dfDeathdata['intDeathsandworkplaces_percent_change_from_baseline3dma'] = dfDeathdata['workplaces_percent_change_from_baseline3dma']*dfDeathdata['deathsper1m_shifted']
